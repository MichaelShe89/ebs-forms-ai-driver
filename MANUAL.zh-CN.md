...
> **先读 §14《排查方法》。** 手册里 39 条故障条目是结论，§14 是产生这些结论的方法。
...
└─ db.py           连接管理与语句审查
                   —— 没有这条通道，规矩 4「界面答不了就查库」根本没法执行
tools/forms-mcp/
├─ jab.py          Java Access Bridge 的 ctypes 绑定。最底层，读界面靠它
├─ bg.py           后台会话：窗口枚举/恢复、节点快照、菜单、按钮、LOV、树行、选择
├─ server.py       JAB 动作封装（_do_action 等）+ MCP 服务器；bg 与 driver 都依赖
├─ keys.py         剪贴板与按键辅助（已修 64 位句柄截断，见 P15）
├─ navigate.py     职责/菜单导航辅助
├─ agent/
...
│     └─ Driver.java       常驻命令服务器
└─ driver/
   ├─ drive.py     Python 驱动端（Forms 类）——你主要用这个
   ├─ smoke.py     落地自检脚本，跑通它再开始做任务
   ├─ nav.py       导航树
   ├─ flow.py      分阶段可续跑编排（模板）
   ├─ witness.py   不干扰的被动自证
   └─ cdp.py       网页端 CDP 客户端
...
> **以 `tools/` 下的文件为准。** 本节是它们的副本，方便只拿到这一个文件的助手
> 也能从零重建。副本会随改动落后 —— 实测已经发生过一次：内嵌的 `drive.py`
> 停在旧版，缺了 §5.1 / §5.2 讲的 `locate()` / `probe()` / `diagnose()`。
> **两边不一致时，信文件，不要信这里。**
...
> 而 `F6` 在有些实例上绑的是 **Clear Record** —— 实测中它抹掉过一张已录入的单据头。
...
`Order Type` 是必填 LOV，而 LOV 快捷键（F9、Shift+Ctrl+F9）在有些实例上一个都
打不开值列表。**不要在界面上继续试快捷键**，去问数据库历史单据用了什么：
